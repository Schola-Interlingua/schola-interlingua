(function (global) {
  ...
  class AudioManager {
    ...
    async playPcm(samples, sampleRate = 22050) {
      ...
      this.playBuffer(context, buffer);
    }

    async playEncoded(data) {
      this.stop();
      const context = this.getContext();
      if (context.state === 'suspended') {
        await context.resume();
      ...
      this.playBuffer(context, audioBuffer);
    }
  ...
  function extractAudio(result) {
    ...
    if (result.wav || result.wave) {
      return {
        samples: result.wav || result.wave,
        sampleRate: result.sampleRate || result.sample_rate || 22050
      };
    }
  ...
  async function speak(text, options = {}) {
    ...
    try {
      if (samples instanceof Uint8Array || samples instanceof ArrayBuffer) {
        await audioManager.playEncoded(samples);
        return;
      }
      await audioManager.playPcm(samples, sampleRate);
    } catch (error) {
      console.warn('No se pudo reproducir el audio TTS.', error);
    }
  }
