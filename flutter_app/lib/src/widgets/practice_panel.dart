  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        const SizedBox(height: 24),
        _TranslationBlocks(items: items),
      ],
    );
  }
}
...
class _TranslationBlocks extends StatefulWidget {
  const _TranslationBlocks({required this.items});

  final List<Map<String, String>> items;

  @override
  State<_TranslationBlocks> createState() => _TranslationBlocksState();
}

class _TranslationBlocksState extends State<_TranslationBlocks> {
  final Map<int, List<TextEditingController>> _controllers =
      <int, List<TextEditingController>>{};
  final Map<int, List<bool?>> _results = <int, List<bool?>>{};

  @override
  void dispose() {
    for (final List<TextEditingController> controllers in _controllers.values) {
  ...
  Widget build(BuildContext context) {
    final AppController controller = AppStateScope.of(context);
    final String lang = controller.selectedLanguage;
    final int totalBlocks = (widget.items.length / 5).ceil();

    return Column(
      children: List<Widget>.generate(totalBlocks, (int blockIndex) {
        final List<Map<String, String>> blockItems = widget.items
            .skip(blockIndex * 5)
            .take(5)
            .toList();
        final List<TextEditingController> controllers = _controllers
            .putIfAbsent(
              blockIndex,
        ...
        final List<bool?> results = _results.putIfAbsent(
          blockIndex,
          () => List<bool?>.filled(blockItems.length, null),
        );
        ...
        return Padding(
          ...
          child: ScholaCard(
            child: Column(
              ...
              children: <Widget>[
                Text(
                  'Traducer — Bloco ${blockIndex + 1}',
                  style: Theme.of(context).textTheme.titleLarge,
                ),
                const SizedBox(height: 16),
                ...List<Widget>.generate(blockItems.length, (int index) {
                  final item = blockItems[index];
                  return Padding(
                    padding: const EdgeInsets.only(bottom: 12),
                    child: Row(
                      children: <Widget>[
                        Expanded(flex: 2, child: Text(item['term'] ?? '')),
                ...
                Wrap(
                  ...
                  children: <Widget>[
                    TextButton(
                      onPressed: () {
                        setState(() {
                          for (int i = 0; i < blockItems.length; i += 1) {
                            final String expected =
                                (blockItems[i][lang] ??
                                    ...
                                    .toLowerCase()
                                    .trim();
                            final String actual = controllers[i].text
                                .toLowerCase()
                                .trim();
                            results[i] = expected == actual;
                          }
                        });
                      },
